...
export const EditTaskModal: React.FC<Props> = ({ isOpen, onClose, task, clientId }) => {
  ...
  // Sync form when task changes (e.g. opening a different task)
  useEffect(() => { setForm(task); setErrors({}); }, [task]);
  ...
  const handleSubmit = (ev: React.FormEvent) => {
    ev.preventDefault();
    if (!validate()) return;
    updateTask(clientId, task.id, { ...form, title: form.title.trim(), description: form.description.trim() });
    onClose();
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Task" maxWidth="600px">
      <form onSubmit={handleSubmit} noValidate>
        <div className="form-group">
          <label className="form-label">Task Title *</label>
          <input type="text" className="form-input" value={form.title} onChange={e => set('title', e.target.value)} style={errors.title ? { borderColor: '#ef4444' } : {}} />
        ...
        <div className="form-group">
          ...
          <textarea className="form-input" rows={3} value={form.description} onChange={e => set('description', e.target.value)} style={{ resize: 'vertical', ...(errors.description ? { borderColor: '#ef4444' } : {}) }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
          <div className="form-group" style={{ marginBottom: 0 }}>
            ...
            <textarea className="form-input" rows={2} value={form.approvalNote} onChange={e => set('approvalNote', e.target.value)} style={{ resize: 'vertical' }} />
          </div>
          <div className="form-group" style={{ marginBottom: 0 }}>
